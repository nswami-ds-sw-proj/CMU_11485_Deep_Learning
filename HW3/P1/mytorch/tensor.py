...
class Tensor:
    ...
    def __str__(self):
        return "{}{}".format(
            str(self.data),
            ", grad_fn={}".format(
                self.grad_fn.__class__.__name__) if self.grad_fn is not None else ""
        )
    ...
    def backward(self):
        ...
        output_grad = Tensor(np.ones((self.data.shape)))
    ...
    def unsqueeze(self,dim=0):
        ...
        return F.Reshape.apply(self, self.shape[:dim] + (1,) + self.shape[dim:])
