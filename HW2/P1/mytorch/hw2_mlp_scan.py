...
class CNN_SimpleScanningMLP:
    ...
    def __init__(self):
        ...
        self.layers = Sequential(self.conv1, ReLU(),
                                 self.conv2, ReLU(), self.conv3, Flatten())
    ...
    def init_weights(self, weights):
        ...
        self.conv1.weight = Tensor(np.transpose(np.reshape(
            w1, (self.conv1.out_channel, self.conv1.kernel_size, self.conv1.in_channel)), (0, 2, 1)), requires_grad=True)

        self.conv2.weight = Tensor(np.transpose(np.reshape(
            w2, (self.conv2.out_channel, self.conv2.kernel_size, self.conv2.in_channel)), (0, 2, 1)), requires_grad=True)
        self.conv3.weight = Tensor(np.transpose(np.reshape(
            w3, (self.conv3.out_channel, self.conv3.kernel_size, self.conv3.in_channel)), (0, 2, 1)), requires_grad=True)
...
class CNN_DistributedScanningMLP:
    ...
    def init_weights(self, weights):
        ...
        self.conv1.weight = Tensor(np.transpose(np.reshape(
            w1[:self.conv1.out_channel, :self.conv1.in_channel*self.conv1.kernel_size], (self.conv1.out_channel, self.conv1.kernel_size, self.conv1.in_channel)), (0, 2, 1)), requires_grad=True, is_parameter=True)

        self.conv2.weight = Tensor(np.transpose(np.reshape(
            w2[:self.conv2.out_channel, :self.conv2.in_channel*self.conv2.kernel_size], (self.conv2.out_channel, self.conv2.kernel_size, self.conv2.in_channel)), (0, 2, 1)), requires_grad=True, is_parameter=True)
        self.conv3.weight = Tensor(np.transpose(np.reshape(
            w3[:self.conv3.out_channel, :self.conv3.in_channel*self.conv3.kernel_size], (self.conv3.out_channel, self.conv3.kernel_size, self.conv3.in_channel)), (0, 2, 1)), requires_grad=True, is_parameter=True)
