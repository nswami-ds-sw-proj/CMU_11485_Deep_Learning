...
def get_final_conv_output_size(layers, input_size):
    ...
    for l in layers:
        if not isinstance(l, Conv1d):
            ...
            assert(isinstance(l, Conv1d))
            if begin:
                begin = False
                output_size = get_conv1d_output_size(input_size, l.kernel_size, l.stride)
            else:
                assert(not begin)
                output_size = get_conv1d_output_size(output_size, l.kernel_size, l.stride)
    return output_size
